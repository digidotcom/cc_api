...
TEST_GROUP(test_ccapi_fs_changed)
{
    void setup()
    {
        ...
        ccapi_filesystem_service_t fs_service = {NULL, ccapi_fs_changed_cb};
        Mock_create_all();
...
TEST(test_ccapi_fs_changed, testOpenForReadNotChanged)
{
    ...
    connector_callback_status_t status;
    connector_file_system_open_t ccfsm_open_data;
    ccapi_fs_file_handle_t * ccapi_fs_handle = th_filesystem_openfile("/tmp/hello.txt", &ccfsm_open_data, CCIMP_FILE_O_RDONLY);
    int fs_context;
