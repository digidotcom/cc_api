...
* Ant 1.9.3 or later (`sudo apt-get install ant` or from [here](http://ant.apache.org/))
* CppUTest 3.6 available at [CppUTest home page](https://cpputest.github.io). Download, extract and execute:`./configure; make; make check; sudo make install;`
...
and mocking interface used. Finally, gcov and gcovr are needed only to build the "coverage" target that output how much of the code is executed by the unit
tests
...
`$ make coverage`
This will build, link and run the test binary and run gcov and gcovr to output the coverage information for all files in "source/" subdirectory
