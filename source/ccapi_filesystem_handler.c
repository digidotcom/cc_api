...
static void ccapi_fs_set_internal_error(ccapi_fs_internal_error_t internal_error, ccimp_fs_errnum_t * const p_ccfsm_errnum)
{
    ...
    *p_ccfsm_errnum = (ccimp_fs_errnum_t)error_handle;
done:
    return;
}
...
static ccimp_status_t ccapi_fs_file_open(ccapi_data_t * const ccapi_data, connector_file_system_open_t * const ccfsm_open_data)
{
    ...
    switch (ccimp_status)
    {
        case CCIMP_STATUS_OK:
        {
            ...
            ccfsm_open_data->handle = (ccimp_fs_handle_t)ccapi_fs_handle;
            break;
        }
        case CCIMP_STATUS_ERROR:
        case CCIMP_STATUS_BUSY:
...
static ccimp_status_t ccapi_fs_dir_open(ccapi_data_t * const ccapi_data, connector_file_system_opendir_t * const ccfsm_dir_open_data)
{
    ...
    {
        ccfsm_dir_open_data->handle = (ccimp_fs_handle_t)ccfsm_dir_open_data->user_context;
...
static ccimp_status_t ccapi_fs_dir_close(ccapi_data_t * const ccapi_data, connector_file_system_close_t * const ccfsm_dir_close_data)
{
    ccimp_status_t ccimp_status = CCIMP_STATUS_ERROR;
...
static ccimp_status_t ccapi_fs_hash_status(ccapi_data_t * const ccapi_data, connector_file_system_stat_t * const ccfsm_hash_status_data)
{
    ...
    else
    {
        ...
        ccimp_hash_algorithm_data.errnum = (ccimp_fs_errnum_t)NULL;
